...
export async function createCriterion(data: {
  ...
  try {
    const [newCriterion] = await db.insert(evaluationCriterion).values({
...
export async function updateCriterion(data: {
  ...
  try {
    const [updated] = await db.update(evaluationCriterion)
      .set({
        name: data.name,
        description: data.description || null,
        maxPoints: data.maxPoints !== undefined ? data.maxPoints : 20,
...
export async function deleteCriterion(id: number) {
  ...
  try {
    const [deleted] = await db.delete(evaluationCriterion)
    ...
    await Promise.all(
      data.scores.map(async (item) => {
        ...
        if (existing.length > 0) {
          await db.update(teamEvaluationScore)
            .set({
