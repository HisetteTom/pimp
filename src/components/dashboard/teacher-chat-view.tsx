...
export function TeacherChatView({ initialTeams }: TeacherChatViewProps) {
  ...
  // Poll list of supervised teams to get fresh unread counts in real-time
  useEffect(() => {
    let active = true;

    async function fetchTeams() {
      try {
        const list = await getSupervisedTeams();
        if (active) {
          setTeams(list);
        ...
        console.error('Failed to fetch supervised teams:', err);
      }
    }
    ...
    return (
      <div className="flex h-96 flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-zinc-200 bg-zinc-50 p-8 text-center dark:border-zinc-800 dark:bg-zinc-900/30">
        ...
        <p className="max-w-sm text-xs text-zinc-400">
          You must be supervising projects with active student teams to access group chats.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-[calc(100vh-12rem)] min-h-[500px] w-full gap-6">
      {/* Left panel: list of supervised teams with Project Filter */}
      ...
      <div className="flex-1">
        {selectedTeam ? (
          <ChatWindow
            ...
            subtitle={`Members: ${selectedTeam.members.join(', ')}`}
          />
        ) : (
